...
A claimed object discloses its join **on its values**, through the same derived-value component: the confirmed facts tie apps into the row's core, whose values carry no mark; a value from an app tied in only by the Synthesizer's judgment is marked guessed; one whose only link no longer holds is marked broken; each value from another app says, on hover or focus, where it came from and what matched, in the Synthesizer's words. A wrong join whose evidence holds — a reused identifier (§6.2) — is not detectable and stays accepted.
