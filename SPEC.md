...
A match claim discloses the join through the shell's **join-disclosure** component, bound to the claim and required by the validator, the derived-value rule applied to entities: quiet, or a plain word, while every match holds; words in the row when a source is absent or a match no longer holds; the sources joined and what each pair matched on, on hover or focus. A wrong join whose evidence holds — a reused identifier (§6.2) — is not detectable and stays accepted.
...
- Entities **appearing** need entity resolution → Synthesizer. On its post-action walk the IntegrityChecker compares each referenced array's key set against the set at accept; a new key is a change-account entry of its own kind, and the re-synthesis is told what appeared, to attach or ignore it. Vanish and appear are the two halves of one walk.
...
| On a shell-painted cell referring to a vendor's entity — every derived value, every line of a join disclosure | **navigate**: scroll, focus and highlight the component bound to the ref's path in that fragment, degrading to the nearest bound ancestor, then to the fragment boundary. Client-local; never journaled | free           |
...
M4   + entity resolution   entity join — a pull request across GitHub · CircleCI · Linear · Gmail   ← differentiator proven
     proves the Planner's join hypothesis and the Synthesizer's checked match claim · relation operators · the join-disclosure rule ·
...
| The synthesize data model — the data model for a2ui composition: a free-form derived data model of formula leaves with a match claim under `match` on each object the Synthesizer judges a join, a shell-catalog tree bound to it, sort declarations, a note — on A2A message metadata, as the synthesis half of the composition contract                                                                                                                                                                      | upstream candidate — the same shape as surface placement above: composition rides the envelope, the A2UI the shell paints stays standard. Resolved client-side before render, so no renderer or component understands a ref                                                                                                                                                                                                                                                                                                           |
...
| A match claim of the synthesis tree must render through the shell's join-disclosure component                                                                                                                                                                            | normative rule, machine-checked — the derived-value rule applied to entities: a claimed join never renders like a plain one; the orchestrator's validator rejects a tree that renders a match claim without it                                                                                                                                                                                                                                                                                                 |
