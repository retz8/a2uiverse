...
- Fires when **every dispatched source has resolved** — arrived, failed, or hit its per-source deadline. The deadline is **patience after the pack**: it starts when every other source has settled and fires after a short wait on the one straggler. It releases synthesis and nothing else; the straggler's dispatch runs on until a longer **hard cap** aborts it into a failure. Under a join hypothesis the home source is exempt: the reserved slot waits for it, and synthesis fires when it lands. The turn's final waits for every dispatch to end; a new utterance cancels the turn.
...
- Recovery re-dispatches only that slot: tier 2. A **failed** source fills its slot the same way, with the shell-painted **failure tile**: the failure said in the client's words at body size, Retry, the vendor's own words beneath. Retry re-dispatches that slot alone and includes it in the merge on arrival.
