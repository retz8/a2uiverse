...
1. A derived data model: a free-form JSON shape of the model's choosing whose every leaf is a formula — one operator the shell catalog declares over refs into partitions — never a literal value, e.g. `min(ref(bh, /items[sku=…]/price), ref(keh, /results[id=…]/cost/amount))`. Entity resolution is expressed as which refs land in the same object, and every object holding refs from two or more sources carries its **match claim**: relation formulas — `equal`, `contains`, … over two refs — under a reserved key, connecting all its sources. The validator checks each relation against the partitions at accept time; the client evaluates them live, so the evidence goes absent with the cells.
2. A synthesis fragment tree in the shell catalog bound to those paths. A formula-bound path renders only through the shell's derived-value component (§14), and a multi-source object renders its match claim through the shell's join-disclosure component (§5.4, §14); literal props in the tree — labels, headings — are presentation.
...
A merged object — refs from two or more sources — discloses the join through the shell's **join-disclosure** component, bound to its match claim and required by the validator, the derived-value rule applied to entities: quiet, or a plain word, while every match holds; words in the row when a source is absent or a match no longer holds; the sources joined and what each pair matched on, on hover or focus. A wrong join whose evidence holds — a reused identifier (§6.2) — is not detectable and stays accepted.
...
| The synthesize data model — the data model for a2ui composition: a free-form derived data model of formula leaves with a match claim in every multi-source object, a shell-catalog tree bound to it, sort declarations, a note — on A2A message metadata, as the synthesis half of the composition contract                                                                                                                                                                      | upstream candidate — the same shape as surface placement above: composition rides the envelope, the A2UI the shell paints stays standard. Resolved client-side before render, so no renderer or component understands a ref                                                                                                                                                                                                                                                                                                           |
...
| A multi-source object of the synthesis tree must render through the shell's join-disclosure component                                                                                                                                                                            | normative rule, machine-checked — the derived-value rule applied to entities: a merged row never renders like a plain one; the orchestrator's validator rejects a tree that renders a multi-source object without it                                                                                                                                                                                                                                                                                                 |
