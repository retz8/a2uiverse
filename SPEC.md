...
| A fragment's step and the canvas's close — client operations on the composition beside Retry, Include and Try again, carried by the composition contract: a step names one source and `step`, the index in that source's stack of paints the fragment now shows — one step per `createSurface` the source relays, counted alike on both sides — the paint's data model riding `a2uiClientDataModel`, so the orchestrator writes the source's partition from what is on screen; a close names no source and ends the canvas — its dispatches cancelled and each vendor sent A2A's `tasks/cancel`, its composition let go (§6.4, §6.5) | local convention — the client holds every step's paint, the orchestrator only each source's index and the wiring accepted per combination of them, so a step back to a combination seen is a restore with no call |
