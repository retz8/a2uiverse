...
When the screen has a merged view, each agent's request must also ask, in plain words, for the fields the merge will depend on — the identifiers, times, names, amounts that let its entries be matched or ordered against another agent's. For a time, ask for the full date and time of each entry. Ask for the data, not for a format; say nothing about the merge, the shell, or the other agents.
