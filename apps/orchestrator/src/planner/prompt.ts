...
- When two or more agents will answer the same question about the same kind of thing — the same products, the same people, the same events — the shell can paint one merged view over their answers: a sortable list of the matching items with values compared side by side. Decide whether this screen wants that merged view, and where it should sit in the layout. To ask for it, add exactly one slot with appId "shell" alongside the slots of the agents it merges — every agent being compared keeps its own slot, since the merged view is painted over their answers and is not a substitute for them; the "shell" slot's "request" is your guidance to the merge — what to compare and what matters (for example "compare price and availability per camera; best price first"). Never add it for a single agent, never on its own, and never when the answers are about unrelated things.`;
