...
The client solves these with a small set of ideas: a **canvas** per question, **surfaces** routed by a **stamp**, a **boundary** around every app's UI, **turns** and **streams beside the turn**, a **trail** of canvases, and a **stack** of screens per app. The next section takes them one at a time.

## Six ideas to hold on to

### 1. A canvas is one question's answer

Every question you ask opens a new **canvas**: its own screen, its own state, its own connection to the orchestrator. The client keeps every canvas for the session. The one on screen is mounted; the others keep running unmounted: their answers still arrive, their merged views still compute, their buttons still finish what they started. Nothing is saved: a reload starts fresh.

On the wire, a canvas is an **A2A context**. The first message of a canvas is sent with no context id; the orchestrator mints one, and every later message on that canvas carries it, so the orchestrator knows which answer a click belongs to. The client and the orchestrator use different words for the same thing: "canvas" is the client's; the orchestrator speaks of a context and its composition.

### 2. Surfaces, slots and the stamp

In A2UI, an agent answers by painting a **surface**: a tree of components from its catalog, plus a JSON **data model** those components bind to. A canvas holds several surfaces at once:
...
### 3. The shell owns the container, the app owns the interior
...
- An **utterance turn**: you ask a question. It opens the canvas.
...
### 5. The trail: every question, kept

The canvases form a tree. A question you ask while viewing an older answer becomes a **child** of that answer. The **trail** draws the tree flat, newest first, with the branches marked. The newest question's canvas is **live**; any other canvas you go back to is a **past canvas**, and it's still a working tab: clicks, presses and sorts land in it, and answers still arrive.
...
```mermaid
flowchart TD
    A["You press Enter in the palette"] --> B["Wiring opens a canvas:<br/>a runtime, and a trail entry that's live and on screen"]
    ...
    G --> H["The stream ends: the turn is over,<br/>the canvas stays live"]
```

**1. A canvas opens.** The palette calls the wiring's `sendUtterance`. It mints a canvas id, creates a **runtime** for it (everything one canvas owns, below), and enters it in the trail as live and on screen. The canvas that was on screen becomes its **parent**; only the session's first question has none.

**2. The question goes out.** The runtime's `open` sends the question as an A2A message with **no context id**. If the canvas has a parent, its context id rides along under the stamp key, so the orchestrator's Planner can read the answer the question was asked from.

**3. The first event names the canvas.** The orchestrator answers at once with the context it minted. The runtime records it, the trail entry takes it, and every later message on this canvas carries it. With the layout, a `paintMeta` part names `shell:main` with the Planner's short title for the canvas (like "Needs attention today" in the trail above), which replaces the truncated question as the trail entry's label. The question itself stays the canvas's heading.
...
**6. The merged view lands.** The last fragment is stamped `source: "shell"`: `shell:synthesis`, the merged view's tree as ordinary A2UI, with its formulas riding beside the stamp on the same event. The client hands the formulas to the canvas's **synthesis session**, which evaluates every cell before React renders, so the table's first frame already has its values. The progress line settles on "Joined Linear issues to GitHub PRs and CircleCI runs".

**7. The stream ends.** The turn is over; the canvas is live and stays so. Everything from here on (a click inside Linear's slot, a sort, a step back) happens inside this canvas.
...
flowchart TD
    ...
    W --> TS["trail store"]
    W --> RT["runtimes, a Map by canvas id"]
    W --> SR["sender resolver"]
    RT --> ONE["createCanvasRuntime, one per canvas"]
    ONE --> ST["canvas store"]
...
- **The entry** (`canvas.tsx`) fetches the list of installed catalogs, resolves each to its runtime catalog and Provider, and builds the shell catalog with the **host relay** (below). This happens once, before any canvas exists.
...
- **A runtime** (`canvas/canvasRuntime.ts`) is everything one canvas owns: its store, its A2A session (the context id), its live `MessageProcessor`, its turn runner, its synthesis session, its fragment history, its binding index and navigator. It runs whether or not it's on screen, and only its close ends it.

**One `MessageProcessor` per canvas, over every installed catalog.** A2UI's processor is the renderer's model: it applies messages and holds surfaces. Each surface carries its own `catalogId`, and the stock library resolves the catalog per surface, so one processor can hold a Primer surface beside a Material 3 surface beside the shell's own. The processor is also the **live registry**: exactly what the agents may see of the canvas when the client sends its data models back.

**The stores are hand-rolled external stores.** The canvas store (`canvas/canvasStore.ts`) and the trail store are closures with `getState`, `subscribe` and small setters, read by React through `useSyncExternalStore`. They're written from non-React code (the turn runner, the A2A callbacks, the replay driver), which is why they aren't component state. The canvas store holds one canvas's state: which surface is on stage, the placement map, the question, the progress facts, the presses in flight. The trail store holds only which canvases exist, which is live, and which is on screen.

### Late binding: the host relay

The shell catalog needs handlers from its host: what to do on a shell action (open the Store), a navigation (a click on a merged cell), a press (Retry), and how to name an app. But the catalog is built at startup, before any canvas exists, and later the handlers must reach **whichever canvas is on screen at that moment**.
...
`CanvasApp` binds the wiring's host while it's mounted, and the wiring's host forwards each call to the canvas on screen. A raise with nothing bound is warned and dropped. With no host at all, the catalog still validates and renders (actions land nowhere, cells aren't clickable, app ids stand in for names), which is exactly what a test or a replay needs.

### The turn runner: validate, then show

`canvas/turn/canvasTurn.ts` is the biggest piece of the client. Every turn enters the canvas through a turn it begins, applies batches into, and ends. Its job is to show paints as they stream, without ever showing a broken one. It works in one of two modes, fixed when the turn starts:

- **Progressive mode**, on an empty canvas: messages apply straight to the live processor, so the paint streams onto the screen as it's written.
...
A press is an operation on the canvas's composition, `{kind, sources}`:
...
The runtime sends it on the canvas's context and answers it on a **side stream** (`runner.beginSideStream()`). What arrives on a side stream is routed by the stamp exactly as a turn's batches are, straight into the live composition. It never touches the stage or the turn in flight. A line's "Retry all" is sent as one Retry per app, each on its own stream.

A press moves through a small set of states in the canvas store, so the UI can show it before any answer arrives:
...
interface TrailEntry {
  ...
  parent?: string;     // the canvas it was asked from
  loading: boolean;
}
```

plus `live` (the newest question's canvas) and `viewing` (the past canvas on screen, or null). The tree is implicit in the `parent` links. Small pure functions read it:

- **`backTarget`**: Back goes **up the branch**, to the canvas the one on screen was asked from, not to the one asked just before it.
- **`isBranch`**: an entry is a branch when its parent isn't the entry just before it in time, meaning it was asked from a past canvas.
- **`newestFirst`**: a sort by `askedAt`, newest first, two questions asked in the same millisecond ordered by which was opened later.
- **`close`**: removing a canvas **re-parents its children to its own parent**, the way deleting a node from a tree links its children to their grandparent. Back keeps working through the gap.
...
**The rail** is the drawer the trail opens in: entries under day headers, "Live" and "Viewing" marks, a quiet mark on a canvas still loading, and on a branch a glyph naming the canvas it was asked from. Hovering an entry shows a **preview**: that canvas's layout surface mounted a second time from its own runtime, laid out at 1120px and scaled to 25%, marked `inert` with pointer events off, and registered under a binding index of its own so a navigation never lands in the copy.

### A past canvas is a tab

Because each canvas is its own runtime, a past canvas needs no special code to keep working. Its streams arrive, its synthesis session evaluates, its presses finish. When you view it, `CanvasView` mounts it; when you leave, it unmounts and keeps running.

<p align="center">
  <img src="../images/past-canvas.png" width="720" alt="A past question's canvas under its Parked band">
  <br>
  <em>A past canvas: parked, stamped with when it was asked, with Ask this again now and Return to live.</em>
</p>

A band over a past canvas reads "Parked · asked at 09:28", the question's time and the canvas's only time. "Ask this again now" sends the same question as a new canvas, asked from this one, and leaves this answer as it was. "Return to live" goes to the newest question. The palette reads "Ask from this view", because whatever you ask becomes this canvas's child.

### The header and the progress line
...
**The progress line is computed, never a model's words.** `canvas/turnProgress.ts` is a pure function from the canvas store to what the line says: "planning" while nothing is planned; a step per app in slot order (a spinner while loading, ✓ once placed, ✕ once failed); then the merge, in sentences built from what the client holds:
...
The only model words in it are the join's nouns ("issues", "PRs", "runs"), which the Planner wrote into the layout. An action inside an app names no action of its own: that app's step spins again until its answer lands. And a canvas-level error, like a request that never arrived, closes the line in the danger tone.
...
1. **Every vendor component registers while it's mounted.** At startup, `decorateCatalog` wraps every component of every vendor catalog in a small component that registers its surface, component model and data context in the canvas's binding index, and unregisters on unmount. The shell catalog isn't decorated: refs only point into apps' data.
...
4. The step goes to the orchestrator on a side stream, carrying the canvas's whole data model, so the orchestrator's copy of the app's data matches what you see.
...
- **Over an empty canvas**, the question goes to an **overlay** above the stage.
...
`retireStage` is the one place a composition leaves its canvas. It deletes the layout **and** every fragment from the canvas's processor, clears placement and promotions, retires the synthesis session, the fragment history and the composition's store state, and ends the side streams. Without that cascade, old fragments would stay in the live registry and ride back out to their apps, as stale data, the next time the client sends its data models. A canvas's close does all of this and then drops the runtime: every stream is cancelled, and the orchestrator is told on the canvas's context so it can cancel the apps' work too.

## The merged view on the client

The merged view is explained end to end in [`synthesis.md`](synthesis.md). The client's part in brief: the merged view's slot renders its surface as **shell content**, in a bare element with no boundary and no attribution, because it's the shell writing on its own page. When `shell:synthesis` arrives, the turn runner hands its formulas to the canvas's **synthesis session** once the surface is live. The session validates them, subscribes to every app surface they read, and runs the pure **BindingEvaluator**, writing the whole result in one write before React renders. A column planned for an app the merge doesn't include yet stays reserved, and its cells are drawn from that app's slot state (`canvas/composition/columnState.ts`): loading, unavailable, or waiting for Include.

## Design decisions

| Decision | What it buys | What it costs |
| --- | --- | --- |
| **A runtime per canvas, kept for the session** | A past answer is a working tab with no special code; closing one is dropping it | Memory grows with every question until a reload |
| **One processor per canvas, over every catalog** | Per-surface catalog resolution is stock library behavior; no dispatch of our own | Every catalog is loaded up front |
...
| **A relay between the shell catalog and the canvas** | The catalog is built once at startup, and its handlers still reach whichever canvas is on screen | A raise with nothing bound is dropped |
| **Side streams for presses and reports** | A press or a report never cancels your turn or touches the trail | Several streams per canvas to track |
...
| `?beat=trail` | Four canvases on two branches: every mark of the rail, and the band on a past canvas |
| `?beat=23`, `?beat=24` | A step back to a screen seen before (restored, no call), and to one never seen |
| `?beat=19` to `?beat=25` | The trail's cases: a tab finishing in the background, an action in a past canvas, Ask this again now, add and drop, closing a loading canvas |
...
| One canvas | `canvas/canvasRuntime.ts`, `canvas/canvasStore.ts` |
...
| Word | Meaning |
| --- | --- |
| **Canvas** | One question's answer on screen, with its own runtime. On the wire, an A2A context |
| **Live** | The newest question's canvas |
| **Past canvas** | Any older canvas you go back to; still a working tab |
| **Trail** | Every canvas of the session, drawn flat by time with its branches |
| **Runtime** | Everything one canvas owns: store, processor, turn runner, synthesis session, history |
