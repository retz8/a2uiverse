...
- **SPEC** — `12eec87`, pushed: §9.2 lets a themed basic catalog append product components for what the basic vocabulary cannot express; §14 gains the appended-component convention (local) and a data-bindable tone on the basic `Text` (upstream candidate).
...
- **Platform** — `abdd985`, pushed: the client on the new catalog tarball. `pnpm verify` green.
