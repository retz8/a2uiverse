...
*Added during the run.* **A declined slot rests on the reason.** The reserved slot used to fold away with nothing said; the reason lived only in the journal. The Synthesizer's reason is now published as the shell's own words in the synthesis slot before the collapse, so the collapsed slot rests on them the way a vendor slot rests on its source's prose (2.9 decision 16). Only a decline speaks — it is the model's judgment in its own words; malformed, skipped and failed are the runtime's and stay silent. Amends phase decision 8 and acceptance item 4; the 4.2 sentence that a decline is indistinguishable from a layout-only turn no longer holds and is left as written there.

*Added during the run.* **The Planner and Synthesizer prompts describe the medium, not rules.** Both prompts were rewritten to say what can be drawn — what a slot is, what the merged view is drawn from, what an entity is — and to leave whether and where to the model, dropping the when/never clauses that had made them rule tables. Under the plain prompts the judgment held: the comparison utterance reserved the merged view four of four, the plain policy question reserved none three of three, and the decline utterance declined when planned — though on that two-part utterance the Planner now sometimes draws two slots for one agent, which the plan check rejects as a broken turn. The one-slot-per-agent constraint is a fact of the medium the prompt states and the runtime enforces; whether the runtime should merge duplicate slots instead of breaking the turn is left to Phase 5's prompt work.
