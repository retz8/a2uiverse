...
### 6. Composition extension: minimal scope, formal home, two language projections

`packages/sdk` ships the composition extension now: extension URI, the `a2uiverse` metadata stamp (source · slot · role), surfaceId namespacing helpers, and the outbound slot request (archetype + budget). Nothing anticipating M2+ (no generation, no deadline fields in the contract). Layout: `packages/sdk/contracts/` holds the normative JSON contract; `packages/sdk/js/` (`@a2uiverse/sdk`, npm) and `packages/sdk/python/` (`a2uiverse-sdk`, PyPI) are hand-written idiomatic projections, each with a contract test that asserts its constants against the JSON — drift is a red build. The Python package joins the pnpm/turbo gate via a thin `package.json` shelling to `uv`; the repo gains a Python toolchain. Escalation to codegen is trigger-gated on the contract outgrowing eyeball verification (realistically M2).
