...
A leaf is `{appId, archetype, request}` — nothing more. Slot names are orchestrator-derived from appId (collision-free with one slot per agent per turn); the Planner never invents identifiers. M8 expands the naming policy for multi-account (memo in `_dev/TODO.md` Phase 12).
