...
An app is `<app dir>/agent`, run as the kit entrypoint with `--mode`. The layout is the scaffolder's own emitted shape, so it holds for anything generated. `agent/pyproject.toml` is the launchability check. The manifest supplies `id` and the agent URL only; it grows no launcher field, and its formalization stays with the Phase 11 sdk schema.
