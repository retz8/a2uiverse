...
4. **Appear, live**: after a workflow is rerun inside the CircleCI fragment on a run a merged row reads, the row tells the truth about CI — through an appeared entry and a re-synthesis, through a value changing in place with no model call, or through the re-synthesis a repainted source the view reads nothing from fires (task 7.9 decision 3, amended during its run).
