...
utterance:
   ...
   8  synthesis (when the plan has a `shell` slot):
        ...
        ◆ synthesizer.synthesize({utterance, request, sources}, partitions) — text out, extract →
          sdk validate → checkSynthesis; one retry with the findings; second failure ⇒ malformed
...
- **Synthesis painter** (`composition/synthesisPainter.ts`): `shell:synthesis`, the shell's second surface. `synthesisParts(tree)` is `createSurface` in the shell catalog plus `updateComponents` carrying the model's components verbatim (task-5.4 decision 10); a re-synthesis paints the same surface again. `synthesisEnvelope` stamps `{source: shell, slot: slot-shell, role: fragment}` and carries the payload `{dataModel, sorts}` under the sdk's `SYNTHESIS_KEY` (`a2uiverseSynthesis`); `synthesisProseEnvelope` carries a decline's reason as a text part under the same stamp, no payload.
...
| Loop      | `MAX_ATTEMPTS = 2`: `buildSynthesisTurn` (sdk) → `SynthesisModel.generate` → `extractSynthesisBlock` → `JSON.parse` → `validateSynthesizeDataModel` (sdk) → `checkSynthesis`; any failure goes back as the turn's `errors` with the failed document as `previous`; a second failure is `malformed`. A decline passes the validator and skips the checklist                                                                                                                                                       |
...
| Prompt    | `prompt.ts`: the sdk's `buildSynthesisSystemPrompt` over the role, the shell catalog's `catalog.json` verbatim and its `guidance.md` (both read once at boot through the package's subpaths); the sdk carries the composition doc and the worked examples. No second copy of the vocabulary here                                                                                                                                                                                                             |
| Checklist | `checkSynthesis.ts`, after the sdk's validator has passed the shape: every component in the shell catalog and the tree through a headless `MessageProcessor` against `SCHEMA_CATALOG`; every named child declared; no `Slot` · `Attribution` · `Frame`; the derived-value rule (phase-5 decision 18) resolved absolutely or through the enclosing template, `SortControl` bound to a declared `/sorts/N`; every operator in the catalog's `OPERATORS`; every ref into a held partition and resolving now — a positional segment and an ambiguous predicate each named as the rule they broke. One line per finding, with its path |
| Operators | `OPERATORS` and `SCHEMA_CATALOG` from `@a2uiverse/shell-catalog/schema` (React-free): the catalog file cannot mark which functions are operators (task 4.3), so the list is the catalog's own export                                                                                                                                                                                                                                                                                                             |
| Tests     | `FakeSynthesizer` / `ThrowingSynthesizer` at the text seam; the checklist and the loop as pure units; the sdk's worked examples validated against the shell catalog; live smoke gated by `A2UIVERSE_SYNTHESIZER_LIVE=1`                                                                                                                                                                                                                                                                                       |
