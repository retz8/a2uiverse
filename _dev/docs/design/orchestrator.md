...
Every request that crosses the hub's boundary leaves a line on stdout (`log.ts`, task 5.7): one per inbound request (kind, task, context, bytes), one per relay start and per relay settle (outcome, ms), one per turn close (outcome, ms). The journal records a turn only when it closes; these lines are the trace of one that does not.
...
utterance:
   ...
   8  synthesis (when the plan has a `shell` slot):
        ...
        declined ⇒ the reason spoken as the shell's words into slot-shell, then slot collapsed
        malformed | model failure ⇒ slot collapsed silently; journal names which
        else payload = {dataModel, sorts}; partitions.snapshot(); state.synthesis = {document, payload};
             paint shell:synthesis into slot-shell — the model's tree verbatim, the payload on metadata
             (nothing else moves)
   9  one turn-final (completed; canceled only when every dispatch cancelled)
  10  journal: plan · dispatch records · surface touches · synthesis {outcome, reason?, synthesizeDataModel?, note?, attempts, changes?, deadAirMs} · close
...
action:   parseSurfaceId(action.surfaceId) → owner; partitions.applyClientDataModel(returning data model);
          ...
          Router/Planner); same pump (materialize, stamp generations); then, if a live synthesis exists
          and checkSynthesisPayload against the partitions finds a ref that no longer resolves ⇒
          re-synthesis inline (step 8) handed the previous document and the change account, before
...
- **State** (`composition/state.ts`): `CompositionState {plan, slots, partitions, arrived, synthesis?, lastSettledAt?}`, held per `clientContextId` in the executor — surviving the turn so a later action or `VALIDATION_FAILED` finds it. `synthesis` is the `LiveSynthesis {document, payload}` once painted: the accepted synthesize data model and the payload the client holds, what the IntegrityChecker guards. Slot states are `pending | failed | collapsed`; `filled` is the client's. `outcomeToSlotState`: failed/timeout → failed · cancelled → collapsed · completed with zero surface touches → collapsed · completed with touches → left to the client (and the source counts as arrived). `synthesisSlot(state)` is the `shell` slot when the plan reserved one.
- **Partitions** (`composition/partitions.ts`): every surface's materialized data model keyed by namespaced id, applied from relayed A2UI ops (`createSurface`, `updateDataModel` at an RFC 6901 path — where `/`, A2UI's default, is the root rather than RFC 6901's empty-string key (task 4.8) — `deleteSurface`) and from the returning client data model. `resolve(ref)` goes through the sdk's resolver (phase decision 23): a value, or absent with its reason (`missing` · `ambiguous` · `null` · `positional`), which the checklist reports by name. Generations (task-4.4 decision 3): before `snapshot()` any change bumps; after it, only an array present in the snapshot and present now with different contents. `generationsOf(changed)` feeds the stamp; nothing on the synthesis path reads them since task 5.10 — resolution is validity.
...
| Synthesis slot | A slot whose `appId` is the reserved `shell` (task-4.4 decision 6): the merged view the shell paints over the other slots, with an archetype and a `request` that is the Planner's prose brief to the Synthesizer — what the view shows, what it orders by, what matters (phase-5 decision 1). Whether and where is the Planner's judgment. When the plan has one, each vendor's request also asks in plain words for the fields the merge depends on — identifiers, and the full date and time of each entry — asking for data, never a format, and naming neither the merge nor the other agents (phase-5 decision 9, task 5.7) |
...
| Prompt         | `prompt.ts`: system role + per-agent card summaries; each slot's `request` is the message its agent receives — prose carrying **all** guidance, size/shape included; the Planner never invents identifiers. The system prompt describes the medium, not rules (task 4.8): what a group and `direction` are, what an agent's slot is, what the merged view is drawn from and that it draws on the other slots on the screen; whether and where are left to the model, and the structural constraints (one slot per agent, one merged view, ≥2 sources beside it) are enforced by the checklist |
...
| Entry           | `{ turnId, clientContextId, at, kind, descriptor, payload?, plan?, synthesis?, dispatch[], surfaces{...}, clientMetadata{keys, dataModelBytes}, outcome, embedding }` — surfaces carry namespaced ids; `outcome` is turn-level; `synthesis {outcome: synthesized\|declined\|malformed\|skipped\|failed, reason?, synthesizeDataModel?, note?, attempts, changes?, deadAirMs}` — the accepted document and its note on `synthesized`, every model call's text and errors in `attempts`, the change account sent on a re-synthesis, and `deadAirMs` from the last source settling to the outcome (phase-4 decision 16, task-5.4 decision 7) |
