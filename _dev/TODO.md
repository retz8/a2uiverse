...
## Phase 6 — The shell as an agent [WIP]
...
- [WIP] **6.3** `sdk` — the `shell:main` payload: a data model of literals beside the tree, the literal-only rule in the validator, the Planner's prompt builder beside the Synthesizer's — the layout prompt hand-written in the orchestrator moves into it — as the shell's UI designer over the catalog and `platform-ui-guidance.md`; each builder shows its author a pruned catalog, only the components and functions that author may use, as upstream's `allowed_components` does, and the component prohibitions in `synthesis-guidance.md` drop once the Synthesizer's catalog is pruned, contract bumped, published (after 6.1; parallel with 6.2)
- [ ] **6.4** Orchestrator — the platform's card: hand-authored skills on the orchestrator's own AgentCard, indexed in the Registry under `shell`; Planner rewrite: `shell:main` authored as text, validated against the catalog and the checklist (one `Slot` per source, merged view iff reserved, one per gap, no `Attribution`/`Frame`/formula/unknown action), one retry; the three readers as the Planner's closed tool set behind one seam; gap entries in the dispatch list; painter wraps every `Slot` in `Attribution` and fills gap slots with the fixed tile; `direction`/`archetype` removed; `Frame` removed from the shell catalog with the painter change that frees it — the Planner sizes slots with the basic `weight`, carried onto each `Slot`'s `Attribution` wrapper; journal records the tree, the tool calls, the gaps (after 6.2 and 6.3; parallel with 6.5)
- [ ] **6.5** Client — `shell:main` literals through the sdk intake; model-placed slots in the placement map; shell-source actions handled locally — ambient notice naming the page and query, reported to the orchestrator as an action turn for the journal; fixtures re-recorded (after 6.2 and 6.3; parallel with 6.4)
- [ ] **6.6** Integration + acceptance — spec decision 10 over the real roster live through the tunnel: the four platform questions, the mixed utterance, the gap, the S1/single-agent/mock-comparison regression with the "side by side" control prompt, the vocabulary checks; first paint measured and recorded, the streaming decision taken against it (after 6.4 and 6.5)
- [ ] **6.7** Design records — the Planner, Registry and painter sections of `orchestrator.md`, the client record's shell-surface and action handling, the shell-catalog record's action set (after 6.6)
...
Store page, App Library, accounts (M10). Opening a trusted page must not unmount the canvas: the client holds its whole session in memory, created when the canvas mounts, and Phase 14's resume after install needs the canvas that raised the capability gap. The lean from 6.2's grill is an overlay over the canvas, addressable by URL; the presentation is decided here.
...
- A canvas action on the live composed screen failed with "Failed to fetch" after about seventy seconds, nothing reaching the vendor or the journal, the orchestrator answering its card throughout (5.7; 4.8 saw the unclosed-turn shape of it). Not reproduced on demand
