...
- The merged view reads as loose lines, not a table (found in 5.7): the guidance doc's table idiom draws with `Row`, which never aligns columns, and every value renders at body size. Try prose guidance toward a per-entry `Card` shape first; a `Table`/`DataList` shell-catalog mapping as the follow-up, under `design-catalog-component`
