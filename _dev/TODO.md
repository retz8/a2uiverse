...
- [WIP] **6.1** Doc edits — SPEC §4.4 (archetypes), §5 (the turn: the Planner's readers and tree), §8 (the gap as a slot the Planner places), §10 (the Planner's row, "never sees data" resolved as never vendor data), §12 M3s line
- [ ] **6.2** `shell-catalog` — the shell's action set declared in the schema (open the Store with an optional query, open the App Library); the layout-authoring guidance the Planner's designer prompt reads beside the catalog (after 6.1; parallel with 6.3)
