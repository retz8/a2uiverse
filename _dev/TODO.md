...
- [ ] **5.9** `shell-catalog` Radix Themes mapping — each basic component implemented on its Radix Themes counterpart, the way `primer-a2ui-adapter` maps onto Primer; the token-themed re-export goes; SPEC §4.2 amendment + §15 row; fixture matrix; parity test (after 5.3; parallel with 5.4, 5.5; before 5.7)
