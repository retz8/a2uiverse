...
- [WIP] **6.2** `shell-catalog` — the shell's action set declared in the schema (open the Store with an optional query, open the App Library); the layout-authoring guidance the Planner's designer prompt reads beside the catalog; if the action form departs from A2UI — anything other than catalog-declared client-side functions invoked by `functionCall`, like the basic catalog's `openUrl` — add its row to SPEC §14 (left out in 6.1) (after 6.1; parallel with 6.3)
- [ ] **6.3** `sdk` — the `shell:main` payload: a data model of literals beside the tree, the literal-only rule in the validator, the Planner's prompt builder as the shell's UI designer over the catalog and guidance, contract bumped, published (after 6.1; parallel with 6.2)
- [ ] **6.4** Orchestrator — the platform's card: hand-authored skills on the orchestrator's own AgentCard, indexed in the Registry under `shell`; Planner rewrite: `shell:main` authored as text, validated against the catalog and the checklist (one `Slot` per source, merged view iff reserved, one per gap, no `Attribution`/`Frame`/formula/unknown action), one retry; the three readers as the Planner's closed tool set behind one seam; gap entries in the dispatch list; painter wraps every `Slot` in `Attribution` and fills gap slots with the fixed tile; `direction`/`archetype` removed; journal records the tree, the tool calls, the gaps (after 6.2 and 6.3; parallel with 6.5)
