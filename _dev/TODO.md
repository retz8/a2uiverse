...
- [WIP] **5.10** `sdk`/`shell-catalog`/orchestrator/client — synthesis refs collapse to one key-based form: compound keys, index refs removed from the contract and rejected by name, re-synthesis triggered by absence, the generation baseline and the `stale` cell state removed, contract v0.4, SPEC §6.2/§6.3/§14 and phase decision 6 amended (after 5.5; before 5.7)
- [ ] **5.11** `sdk` — the Synthesizer's worked examples rewritten against task 5.6's recorded shapes: heterogeneous kinds, opaque ids, a source whose time is not ISO (after 5.6 and 5.10; before 5.7)
