...
- **The resolution kit** — pointer and predicate resolution against a data model (a value, or absent: missing, ambiguous, null, or positional — a segment that indexes into an array, which is not a ref); the model walk enumerating every formula leaf and every ref. Resolution is validity: a ref is good while its key resolves.
